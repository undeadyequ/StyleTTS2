...
class Decoder(nn.Module):
    def __init__(self, noise_channels, cond_channels, hidden_channels, out_channels, filter_channels, dropout=0.1, n_layers=1,
        ...
        # upsample mels -> 2 * mels
        self.mel_upsample = nn.Sequential(
            ResBlk1dUp(out_channels, out_channels, upsample=True),
            ResBlk1dUp(out_channels, out_channels),
            ResBlk1dUp(out_channels, out_channels))
        self.to_out = nn.Sequential(weight_norm(nn.Conv1d(out_channels, out_channels, 1, 1, 0)))
    ...
    def forward(self, t, x, mask, mu, c, seq_style=None, p_mask=None, return_attn_map=True, q_f_pos=None, k_f_pos=None):
        ...
        # Upsampling mel
        output = self.mel_upsample(output)
