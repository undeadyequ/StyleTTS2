...
class MultiHeadAttention(nn.Module):
    ...
    def attention(self, query, key, value, mask=None):
        ...
        output, attn_map = scaled_dot_product_attention(query, key, value, attn_mask=mask,
...
class DiTConVBlockUncond(nn.Module):
    """
    A DiT block with adaptive layer norm zero (adaLN-Zero) conditioning.
    """
    ...
    def forward(self, x, x_mask):
        ...
        attn_out, attn_map = self.attn(x.transpose(1, 2).transpose(1, 2), attn_mask)
        x = x + attn_out * x_mask
        x = x + self.mlp(x.transpose(1, 2).transpose(1, 2), x_mask)
        return x
