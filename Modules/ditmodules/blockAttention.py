...
class MultiHeadAttentionCross(nn.Module):
    ...
    def attention(self, query, key, value, mask=None, q_f_pos=None, k_f_pos=None, refenh_ind_dur=None, synenh_ind_dur=None):
        ...
        if self.phoneme_RoPE == "phone" or self.phoneme_RoPE == "sel":
            ...
            query = self.query_rotary_pe(query, seq_dur=q_f_pos)  # [b, n_head, t, c // n_head]
...
class MultiHeadAttention(nn.Module):
    ...
    def attention(self, query, key, value, mask=None):
        ...
        #output, attn_map = scaled_dot_product_attention(query, key, value, attn_mask=mask, dropout_p=self.p_dropout if self.training else 0)

        output = output.transpose(2, 3).contiguous().view(b, d, t_t)  # [b, n_h, t_t, d_k] -> [b, d, t_t]
        return output, None

class RotaryPositionalEmbeddingsManipulate(nn.Module):
    """
    ## RoPE module
    ...
    def _build_cache(self, x: torch.Tensor):
        ...
        theta = 1.0 / (self.base * 0.01 ** (torch.arange(0, self.d, 2).float() / self.d)).to(x.device)

        # Create position indexes `[0, 1, ..., seq_len - 1]`
        seq_idx = torch.arange(seq_len, device=x.device).float().to(x.device)
        ############ CHECK CODE ############
        #seq_idx[:min(40, len(seq_idx))] += 40
        #print(seq_idx)
...
def scaled_dot_product_attention(query, key, value, attn_mask=None, dropout_p=0.0, is_causal=False, scale=None,
    ...
    ############ Test: Enhancement ##############
    #guide_matrix = make_guided_attention_masks(ilens=torch.tensor([attn_weight.size(2)]).unsqueeze(0), olens=torch.tensor([attn_weight.size(3)]).unsqueeze(0), chunksize=6)  # (b, ilens_max, olens_max)
    #guide_matrix_mask = guide_matrix < 0.0001

    #torch.set_printoptions(threshold=10_000)
    #print(guide_matrix[0, :20, :20].detach().cpu())
    #save_plot(guide_matrix_mask[0].detach().cpu(), f'./monoMask.png')

    ############ Test: Enhancement ##############
    #ref_enh_start, ref_enh_dur = 33, 30
    #tgt_enh_start, tgt_enh_dur = 15, 30
