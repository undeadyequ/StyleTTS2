...
import numpy as np
from Modules.diffusionmodules.ema import *
...
from utilities.model_util import get_vocoder
...
class AutoencoderKL(pl.LightningModule):
    def __init__(
        ...
        if self.image_key == "fbank":
            self.vocoder = get_vocoder(None, "cpu", num_mel, sampling_rate=sampling_rate)
    ...
    def decode_to_waveform(self, dec):
        from utilities.model_util import vocoder_infer
        if self.image_key == "fbank":
            dec = dec.squeeze(1).permute(0, 2, 1)
            if self.sampling_rate == 16000:
                wav_reconstruction = self.hifi_16_vocoder(dec).cpu().detach().numpy()
            elif self.sampling_rate == 24000:
                vocoder = get_hifig_vocoder("/home/rosen/ckpt/styletts/Vocoder/LibriTTS")
                wav_reconstruction = vocoder(dec).cpu().detach().numpy()
                wav_reconstruction = (wav_reconstruction.cpu().numpy() * 32768).astype("int16")
            else:
                wav_reconstruction = vocoder_infer(dec, self.vocoder)  # original
    ...
    def get_input(self, batch):
        fname, text, label_indices, waveform, fbank = (
            batch["fname"],
            batch["text"],
            batch["label_vector"],
            batch["waveform"],
        ...
        ret["fbank"], ret["fname"], ret["waveform"] = (
            fbank.unsqueeze(1),
            fname,
...
if __name__ == "__main__":
    ...
    config_yaml = "drawspeech/config/diverse_data_vae_autoencoder/16k_64.yaml"
