...
import datetime
from drawspeech.utilities.model_util import save_attn_dict

from drawspeech.utilities.model_util import (
    exists,
    default,
    mean_flat,
    count_params,
...
from drawspeech.utilities.diffusion_util import (
    make_beta_schedule,
    extract_into_tensor,
    noise_like,
)

from drawspeech.utilities.tools import (
    wav_mask_to_latent_mask,
    avg_pool_with_mask,
)

from drawspeech.modules.diffusionmodules.ema import LitEma
from drawspeech.modules.diffusionmodules.distributions import (
    normal_kl,
    DiagonalGaussianDistribution,
)


#from drawspeech.modules.latent_diffusion.ddim_dit import DDIMSampler
from drawspeech.modules.latent_diffusion.ddim import DDIMSampler

from drawspeech.modules.latent_diffusion.plms import PLMSSampler
...
class LatentDiffusion(DDPM):
    ...
    def get_learned_conditioning(self, xc, key, unconditional_cfg):
    ...
    def generate_sample_with_extra_out(
        ...
        with self.ema_scope("Plotting"):
            for i, batch in enumerate(batchs):
                ...
                mel_masks = ori_c["noncond_mel_mask"]
                ...
                for j in range(len(batch["fname"])):
                    ...
                    waveform = self.mel_spectrogram_to_waveform(mel_rm_silence.unsqueeze(0),
                                                                savepath=waveform_save_path, bs=None, name=fnames, save=False)
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, cond_dict: dict = {}):
        ...
        # x is the latent from get_first_stage_encoding and has shape [b, latent_embed_dim, latent_t_size, latent_f_size]
        xc = x
        ...
        for key in conditional_keys:
            if "concat" in key:
                if cond_dict[key].dim() == 3:
                    xc = torch.cat([xc, cond_dict[key].unsqueeze(1)], dim=1)
                elif cond_dict[key].dim() == 4:
                    xc = torch.cat([xc, cond_dict[key]], dim=1)
                else:
                    raise NotImplementedError()
            elif "film" in key:
                if y is None:
