...
#from Modules.conditional_models import *
import datetime
from utilities.model_util import convert_3d_to_4d, convert_4d_to_3d, save_attn_dict
from utilities.guide_mask import make_guided_attention_masks2
from utilities.vis import save_plot
import time
from utilities.model_util import get_hifig_vocoder
...
from utilities.tools import (
    wav_mask_to_latent_mask,
    avg_pool_with_mask,
)

from Modules.diffusionmodules.ema import LitEma
from Modules.diffusionmodules.distributions import (
    normal_kl,
    DiagonalGaussianDistribution,)

from Modules.latent_diffusion.ddim_dit import DDIMSampler
from Modules.latent_diffusion.plms import PLMSSampler
...
class DDPM(pl.LightningModule):
    ...
    def __init__(
        ...
        # conditioning_key == [film_clap_cond1, crossattn_flan_t5]
    ...
    # compute x_0 conditioned on x_t and the predicted noise
    ...
    # compute the mean and variance of q(x_{t-1} | x_t, x_0), in which x_0 is reconstructed
    ...
    # sample x_{t-1} conditioned on the predicted mean and variance of q(x_{t-1} | x_t, x_0)
    @torch.no_grad()
    ...
    # compute x_t conditioned on x_0 and t
    ...
    def get_input(self, batch, k):
        ...
        fname, text, label_indices, fbank = (
            batch["fname"],
            batch["text"],
            batch["label_vector"],
            #batch["waveform"],
...
class LatentDiffusion(DDPM):
    ...
    def __init__(
        ...
        use_tts_data_for_training=False,
        learn_monoAttn=False,
        mask_delta=0.5,
        *args,
        **kwargs,
    ):
        ...
        self.use_tts_data_for_training = use_tts_data_for_training
        self.restarted_from_ckpt = False
        if ckpt_path is not None:
            self.init_from_ckpt(ckpt_path, ignore_keys)
            self.restarted_from_ckpt = True
        self.TRUE_MARK = True
        self.learn_monoAttn = learn_monoAttn
    ...
    def get_learned_conditioning(self, xc, key, z, unconditional_cfg):
        # key refer to cond_model_key
        assert key in self.cond_stage_model_metadata.keys()

        # Classifier-free guidance
        if not unconditional_cfg:
            c = self.cond_stage_models[self.cond_stage_model_metadata[key]["model_idx"]](xc, z)
            #c = self.cond_stage_models[self.cond_stage_model_metadata[key]["model_idx"]](xc)

        else:
            # when the cond_stage_key is "all", pick one random element out
            if isinstance(xc, dict):
                xc = xc[list(xc.keys())[0]]

            if isinstance(xc, torch.Tensor):
                batchsize = xc.size(0)
            elif isinstance(xc, list):
                if isinstance(xc[0], torch.Tensor):
                    batchsize = xc[0].size(0)
                else:
                    batchsize = len(xc)
            else:
                raise NotImplementedError()
    ...
    def get_input(
        ...
        if len(self.cond_stage_model_metadata.keys()) > 0:
            unconditional_cfg = False  # return conditional information
            if self.conditional_dry_run_finished and self.make_decision(unconditional_prob_cfg):
                unconditional_cfg = True  # return unconditional information
            for cond_model_key in self.cond_stage_model_metadata.keys():
                ...
                if cond_stage_key != "all":
                    # If the current conditional model need multiple conditioning signal and the signal is from previous conditional model
                    if isinstance(cond_stage_key, list):
                        xc = {}
                        for k in cond_stage_key:
                            if k in cond_dict.keys():
                                xc[k] = cond_dict[k]
                            else:
                                _xc = super().get_input(batch, k)
                                if type(_xc) == torch.Tensor:
                                    _xc = _xc.to(self.device)
                                xc[k] = _xc
                    else:
                ...
                c = self.get_learned_conditioning(xc, z=z, key=cond_model_key, unconditional_cfg=unconditional_cfg)
        ...
        if return_latent_mask:
            wav_mask = batch['wav_mask']
            latent_mask = wav_mask_to_latent_mask(wav_mask, self.latent_t_size)
            out += [latent_mask]
    ...
    def mel_spectrogram_to_waveform(
        ...
        if self.sampling_rate == 16000:
           waveform = self.hifi_16_vocoder(mel)
        else:
    ...
    def hifi_16_vocoder(self, mels):
        ...
        return  hifi_gan.decode_batch(mels)
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False):
        #cond = self.reorder_cond_dict(cond)   # cond (tuple) is from styleTTS2
        x_recon, extra_info = self.model(x_noisy, t, cond_dict=cond)

        if isinstance(x_recon, tuple) and not return_ids:
            return x_recon[0], extra_info
        else:
            return x_recon, extra_info

    def p_losses(self, x_start, cond, t, noise=None, loss_mask=None):
        noise = default(noise, lambda: torch.randn_like(x_start))
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        model_output, extra_info = self.apply_model(x_noisy, t, cond)
        ...
        loss_simple = avg_pool_with_mask(loss_simple, loss_mask, time_dim=2, reduce_dim=(1, 2, 3))
        ...
        if self.learn_monoAttn:  # TEMP CODE
            mel_interp_lens = cond[-2]
            cross_attns = extra_info[0]
            blk, batch, head, ql, kl = cross_attns.shape
            guide_matrix = make_guided_attention_masks2(ilens=mel_interp_lens, olens=mel_interp_lens, max_len=ql,
                                                        base_sigma=self.mask_delta, eps=0.002)  # (b, ilens_max, olens_max)
            guide_matrix = guide_matrix.unsqueeze(1).unsqueeze(0).repeat(blk, 1, head, 1, 1)
            monAttn_loss = torch.mean(cross_attns * guide_matrix)
            ######### Check ########
            if self.TRUE_MARK:
                #print("monotonic attn guiding Enabled!", guide_matrix[0, 0, 0].detach().cpu())
                save_plot(guide_matrix[0, 0, 0].detach().cpu(), "monoMask_guassion.png")
                self.TRUE_MARK = False

            mono_weight = 1
            loss_dict.update({f"{prefix}/loss_monoAttn": monAttn_loss})
            loss += monAttn_loss * mono_weight

        loss = self.l_simple_weight * loss.mean()

        loss_vlb = self.get_loss(model_output, target, mean=False)
        loss_vlb = avg_pool_with_mask(loss_vlb, loss_mask, time_dim=2, reduce_dim=(1, 2, 3))
    ...
    def sample_log(
        ...
        **kwargs,
        ...
        if mask is not None:
            ...
            shape = (self.channels, cond[1].size(-1), self.latent_f_size)  # self.latent_t_size ->

        intermediate = None
        if ddim and not use_plms:   # Return cross_attns only on ddim (Keep other method clean)
            print("Use ddim sampler")

            ddim_sampler = DDIMSampler(self)
            samples, intermediates, extra_info = ddim_sampler.sample(
                ddim_steps,
                batch_size,
                shape,
                cond,
        ...
        return (samples, intermediate), extra_info
    ...
    def generate_sample(
        ...
        **kwargs,
        ...
        # Generate n_gen (n_candidates_per_samples) times and select the best
        ...
        with self.ema_scope("Plotting"):
            for i, batch in enumerate(batchs):
                ...
                for cond_key in c.keys():
                    ...
                    elif isinstance(c[cond_key], tuple):   # TEMP CODE
                        pass
                    else:
                ...
                (samples, _), cross_attns = self.sample_log(
                    cond=c,
                    batch_size=batch_size,
                    x_T=x_T,
                    ddim=use_ddim,
                ...
                samples = z if recons else samples
                mel = self.decode_first_stage(samples)
                #mel = self.decode_first_stage(z)
    ...
    @torch.no_grad()
    def generate_sample_with_extra_out(
            self,
            batchs,
            ddim_steps=200,
            ddim_eta=1.0,
        ...
        with self.ema_scope("Plotting"):
            for i, batch in enumerate(batchs):
                ...
                (samples, _), extra_info = self.sample_log(
                    cond=c,
                    batch_size=batch_size,
                    x_T=x_T,
                    ddim=use_ddim,
                ...
                print(f"Cost time {cost_time} for batch_size: {batch_size}")

                cross_attns, mel_interp_lens, mel_masks = extra_info

                mel = self.decode_first_stage(samples)
                # mel = self.decode_first_stage(z)
                #waveform = self.mel_spectrogram_to_waveform(mel, savepath=waveform_save_path, bs=None, name=fnames, save=False)

                #  save speech/txt, and attn info
                #  attn_json: {spk/emo/model: {speech_id:[], syn_phonemes:[], ref_phonemes:[], q_dur:[], k_dur:[]}
                model_name = name.split("/")[-2]
                wav_names = []
                for j in range(len(batch["fname"])):
                    # get extra info
                    fname = batch["fname"][j]
                    if "ESD" in fname: # /hdd/ESD/0019/Angry/train/0019_000414.wav VS wavs/0015/0015_000287.wav
                        spk, emo = fname.split("/")[3], fname.split("/")[4]
                    elif "LibriTTS" in fname: # /hdd/LibriTTS_16k/train-clean-360/2790/142824/2790_142824_000046_000000.wav
                        spk, emo = fname.split("/")[4], "Neutral"
                    else:
                        raise IOError(f"{fname} is not supported")

                    ref_id, syn_id = batch["datum"]["r_id"][j], batch["datum"]["t_id"][j]
                    speech_id = f'spk{spk}_{emo}_ref{ref_id}_syn{syn_id}'
                    syn_phonemes = batch["datum"]["phonemes"][j].split(" ")  # rm "{sil" and "sil}"
                    ref_phonemes = batch["datum"]["ref_phonemes"][j].split(" ")
                    k_dur = batch["phoneme_duration"][j]  # get from add_on
                    k_dur = ref_dur = k_dur[torch.nonzero(k_dur, as_tuple=True)]
                    q_dur = c_orig["predit_dur"][j]
                    q_dur = syn_dur = q_dur[torch.nonzero(q_dur, as_tuple=True)]
                    assert len(ref_phonemes) == len(ref_dur)
                    assert len(syn_phonemes) == len(syn_dur)

                    transcription = batch["datum"]["transcription"][j]
                    ...
                    wav_names.append(speech_id)
                    txt_f = f'{waveform_save_path}/{speech_id}.lab'
                    with open(txt_f, "w") as file1:
                        file1.write(transcription)

                    # generate wav
                    mel_rm_silence = mel[j][:, :sum(mel_masks[j]), :]
                    waveform = self.mel_spectrogram_to_waveform(mel_rm_silence.unsqueeze(0), savepath=waveform_save_path, bs=None, name=fnames, save=False)
                    self.save_waveform(waveform, waveform_save_path, name=[speech_id])

                    # save attn
                    if save_attn:
                        attn_path = f'{waveform_save_path}_attn//{speech_id}.npy'
                        np.save(attn_path, cross_attns[:, :, j, ...].cpu().numpy())   ## [time, block_n, batch, head, t_t, t_s]

                ## save speech/txt
                # self.save_waveform(waveform, waveform_save_path, name=fnames)
                #self.save_waveform(waveform, waveform_save_path, name=wav_names)
        return (waveform_save_path, attn_dict)


class DiffusionWrapper(pl.LightningModule):
    def __init__(self, diff_model_config, conditioning_key, dim_in=512, residual_dim=64):
        super().__init__()
        ...
        pe_emb_dim = 256
        asr_res_dim = 160  # to concat with z
        ...
        self.N_conv = nn.Sequential(
            ResBlk1d(1, residual_dim, normalize=True, downsample=True),   # F0 and N shrunk to half to match with asr
            weight_norm(nn.Conv1d(residual_dim, 1, kernel_size=1)),
            nn.InstanceNorm1d(1, affine=True)
        )
        ...
        self.asr_res = nn.Sequential(
            weight_norm(nn.Conv1d(dim_in, asr_res_dim, kernel_size=3, padding=1)),  # keep same length of ouput
            nn.InstanceNorm1d(asr_res_dim, affine=True)
        )
        #self.mu_linear = torch.nn.Linear(20, 160)
    ...
    def forward(self, x, t, cond_dict: dict = {}):
        ...
        x: [b, c * d, l]
        mu: (b, 512, L)
        pitch, energy: (b, 1, L*2)
        sg: (b, 128)
        """
        x = x.contiguous()
        t = t.contiguous()
        mu, pitch, energy, sg, z_len, z_mask = cond_dict  # (32, L, 20)   ??? can dim = 160?

        # Embed pe
        #pitch = self.F0_conv(pitch)
        #energy = self.N_conv(energy)
        r = self.pe_encode(torch.cat([pitch, energy], dim=1))
        #r = torch.concat([pitch, energy], dim=2)
        #seq_style = self.pitch_embed(r).transpose(1, 2)  # (32, L, 40) -> (32, 256, L)  p_mask???

        # Upsample asr
        mu = F.interpolate(mu, scale_factor=2, mode="nearest")
        mu = self.asr_res(mu)
        # mu = self.mu_linear(mu).transpose(1, 2)

        # convert z
        x = convert_4d_to_3d(x)  # (32, 8, L, 20)  -> (32, 160, L)

        # full true x
        x_mask = torch.ones([x.size(0), 1, x.size(-1)]).to(x.device)
        p_mask = x_mask
        #mel_interp_mask = torch.unsqueeze(mel_interp_mask.to(torch.float), 1)

        out, cross_attn = self.diffusion_model(t, x, x_mask, mu, sg, r, p_mask)  # (32, 160, L)   # estimator no change
        out = convert_3d_to_4d(out, channel=8, converted_z_dim=160)

        extra_info = [cross_attn, z_len, z_mask]
