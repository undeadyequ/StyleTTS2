...
from Modules.latent_diffusion.pre_dit_modules import ResBlk1dUp
...
class CFMDecoder(torch.nn.Module):
    def __init__(self, noise_channels, cond_channels, hidden_channels, out_channels, filter_channels, n_heads, n_layers,
        ...
        self.pe_encode = nn.Sequential(ResBlk1d(2, residual_dim, normalize=True),
                                       weight_norm(nn.Conv1d(residual_dim, pe_emb_dim, kernel_size=1)),
                                       nn.InstanceNorm1d(pe_emb_dim, affine=True))
        self.asr_res = nn.Sequential(
            ResBlk1dUp(dim_in, dim_in, upsample=True),
            weight_norm(nn.Conv1d(dim_in, asr_res_dim, kernel_size=1)),  # keep same length of output
