...
from Modules.ditmodules.estimator_up_mel import Decoder
...
class CFMDecoder(torch.nn.Module):
    def __init__(self, noise_channels, cond_channels, hidden_channels, out_channels, filter_channels, n_heads, n_layers,
        ...
        self.x_conv = nn.Sequential(
            ResBlk1d(asr_res_dim, asr_res_dim, normalize=True, downsample=True),
            weight_norm(nn.Conv1d(asr_res_dim, 1, kernel_size=1)),
    ...
    def forward(self, mu, mask, n_timesteps, temperature=1.0, c=None, seq_style=None, p_mask=None, solver=None, cfg_kwargs=None,
        ...
        """
        ### Downsample pe
        N, F0 = seq_style[:, 0, :], seq_style[:, 1, :]  #### Need CHECK
        N = self.N_conv(N.unsqueeze(1))
        F0 = self.F0_conv(F0.unsqueeze(1))
        seq_style = self.pe_encode(torch.cat([F0, N], dim=1))
