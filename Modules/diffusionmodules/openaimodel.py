...
from drawspeech.modules.diffusionmodules.attention import SpatialTransformer
...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb, context_list=[], mask_list=[]):
        # if extra_sa_layer is ture, the first spatial transformer block does not have context
...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if mask is None:
            ...
            return checkpoint(self._forward, (x, mask), self.parameters(), True)

    def _forward(self, x, mask=None):
        b, c, *spatial = x.shape
        x = x.reshape(b, c, -1).contiguous()
        qkv = self.qkv(self.norm(x)).contiguous()
        h = self.attention(qkv, mask=mask).contiguous()
...
class QKVAttentionLegacy(nn.Module):
    ...
    def forward(self, qkv, mask=None):
        ...
        if mask is not None:
            ...
            max_neg_value = -th.finfo(weight.dtype).max
            mask = repeat(mask, "b j -> (b h) () j", h=self.n_heads)
            weight.masked_fill_(~(mask == 1), max_neg_value)
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.extra_film_condition_dim = eval(extra_film_condition_dim) if isinstance(extra_film_condition_dim, str) else extra_film_condition_dim
        self.use_checkpoint = use_checkpoint
