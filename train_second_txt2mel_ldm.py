...
import json
from utilities.model_util import convert_4d_to_3d, convert_3d_to_4d, convert_mel2z
...
def main(config_path):
    ...
    # load pretrained ASR/F0/PL-BERT model
    ...
    # DP -> THIS CODE CAUSED problem, -> style_encoder parameter disapeared ??? I donot kown why???
    #for key in model:
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            for bib in range(len(mel_input_length)):
                ...
                p_en_z.append(d_z[bib, :, random_start_z:random_start_z + z_len])
            ...
            #mel_rec = model.decoder(gd_z_4d, en, F0_fake_z, N_fake_z, s_z)
            optimizer.zero_grad()
            ...
            g_loss = loss_params.lambda_mel * loss_mel + \
                     loss_params.lambda_F0 * loss_F0_rec_z + \
                     loss_params.lambda_ce * loss_ce + \
                     loss_params.lambda_norm * loss_norm_rec_z + \
                     loss_params.lambda_dur * loss_dur + \
                     loss_params.lambda_ldm * loss_ldm_mono_vlb + \
                     loss_params.lambda_sty * loss_sty + \
                     loss_params.lambda_diff * loss_diff

            running_loss += loss_ldm_mono_vlb.item()
            g_loss.backward()
            ...
            if (i + 1) % log_interval == 0:
                logger.info(
                    'Epoch [%d/%d], Step [%d/%d], Loss: %.5f, Dur Loss: %.5f, CE Loss: %.5f, Norm Loss: %.5f, F0 Loss: %.5f, LDMdiff Loss: %.5f, Sty Loss: %.5f, Diff Loss: %.5f'
                    % (epoch + 1, epochs, i + 1, len(train_list) // batch_size, running_loss / log_interval,
                       loss_dur, loss_ce, loss_norm_rec_z.item(), loss_F0_rec_z.item(), loss_ldm_mono_vlb, loss_sty, loss_diff
                       ))

                writer.add_scalar('train/LDMdiff_mean', running_loss / log_interval, iters)
                writer.add_scalar('train/ldmdiff_loss', loss_ldm_mono_vlb, iters)
                writer.add_scalar('train/ce_loss', loss_ce, iters)
                writer.add_scalar('train/dur_loss', loss_dur, iters)
                writer.add_scalar('train/norm_loss', loss_norm_rec_z, iters)
                writer.add_scalar('train/F0_loss', loss_F0_rec_z, iters)
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(val_dataloader):
                ...
                try:
                    ...
                    # get z and z_lengths, ref_z and ref_z_lengths
                    z, z_length = convert_mel2z(mels, model, mel_input_length)
                    ref_z, ref_z_length = convert_mel2z(ref_mels, model, mel_input_length)
                    ...
                    s_z = torch.stack(ss_z).squeeze()
                    gs_z = torch.stack(gs_z).squeeze()
                    s_trg_z = torch.cat([s_z, gs_z], dim=-1).detach()

                    bert_dur = model.bert(texts, attention_mask=(~text_mask).int())
                    d_en = model.bert_encoder(bert_dur).transpose(-1, -2)
                    d_z, p_z = model.predictor(d_en, s_z, input_lengths, s2s_attn_mono, text_mask)
                    ...
                    for bib in range(len(mel_input_length)):
                        ...
                        p_en_z.append(p_z[bib, :, random_start_z:random_start_z + z_len])
                        en_z.append(asr_z[bib, :, random_start_z:random_start_z + z_len])
                        gt_z.append(z[bib, :, (random_start_z * 2):((random_start_z + z_len) * 2)])
                    ...
                    s_z = model.predictor_encoder(gt_z.unsqueeze(1))

                    F0_fake_z, N_fake_z = model.predictor.F0Ntrain(p_en_z, s_z)
                    ...
                    loss_F0_z = F.l1_loss(F0_real_z, F0_fake_z) / 10

                    loss_test += (loss_diff_mono_vlb).mean()
                    loss_align += (loss_dur).mean()
                    loss_f += (loss_F0_z).mean()
        ...
        logger.info(
            'Validation loss: %.3f, Dur loss: %.3f, F0 loss: %.3f' % (loss_test / iters_test, loss_align / iters_test,
                                                                      loss_f / iters_test) + '\n\n\n')
        print('\n\n\n')
        writer.add_scalar('eval/ldmdiff_loss', loss_test / iters_test, epoch + 1)
        ...
        if epoch < joint_epoch:
            ...
            with torch.no_grad():
                for bib in range(len(asr)):
                    ...
                    mel_pred = model.decoder(gd_z_4d, cond)
