...
from utilities.tools import load_json, data_filter, wav_mask_to_latent_mask
from utilities.dataset_plugin import *
...
class AudioDataset(Dataset):
    def __init__(
        ...
        self.build_dsp() # digital signal processor
    ...
    def __getitem__(self, index):
        ...
        data = {
            ...
            "contain_caption": self._is_contain_caption(datum) or self._is_contain_label(datum),
            "wav_mask": wav_mask,
            "mel_mask": wav_mask_to_latent_mask(wav_mask, log_mel_spec.size(0)),
        }

        # data = {
        #     "fname": self.data[index]["wav"],
        #     "text": "",
        #     "label_vector": "",
        #     "waveform": "",
        #     "stft": "",
    ...
    def feature_extraction(self, index):
        ...
        while True:
            try:
                ...
                audio_file = datum["wav"]
                if not os.path.exists(audio_file) and "reference" in datum.keys():
                    audio_file = datum["reference"]
                ...
                (
                    ...
                    random_start,
                    wav_mask,
                ) = self.read_audio_file(audio_file)
                mix_datum = None
                ...
                print("Error encounter during audio feature extraction: ", e, audio_file)
                continue
    ...
    def build_setting_parameters(self):
        ...
        self.latent_t_size = self.config["variables"]["latent_t_size"]
    ...
    def build_dataset(self):
        ...
        if type(self.dataset_name) is str:
            ...
            self.data = data_filter(data_json["data"])
    ...
    def resample(self, waveform, sr):
        print("Resample from %s to %s" % (sr, self.sampling_rate))
    ...
    def pad_wav(self, waveform, target_length):
        ...
        if waveform_length == target_length:
            wav_mask = torch.ones((target_length,))
            return waveform, wav_mask
        ...
        wav_mask = torch.zeros((target_length,))
        wav_mask[rand_start : rand_start + waveform_length] = 1

        return temp_wav, wav_mask
    ...
    def read_wav_file(self, filename):
        ...
        # keep the original waveform to ensure alignment with the metadata (pitch, energy, duration)
        ...
        waveform, wav_mask = self.pad_wav(
            waveform, target_length=int(self.sampling_rate * self.duration)
        )
        return waveform, random_start, wav_mask

    def read_audio_file(self, filename, filename2=None):
        if os.path.exists(filename):
            waveform, random_start, wav_mask = self.read_wav_file(filename)
        else:
            print(
        ...
        return log_mel_spec, stft, waveform, random_start, wav_mask
