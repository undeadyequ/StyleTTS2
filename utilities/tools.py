...
def wav_mask_to_latent_mask(wav_mask, latent_t_size):
    if wav_mask.dim() == 1:
        extra = wav_mask.size(0) % latent_t_size
        if extra > 0:
            wav_mask = wav_mask[:-extra]
        wav_mask = wav_mask.view(latent_t_size, -1)
        latent_mask = wav_mask.all(-1).to(wav_mask.dtype)
    elif wav_mask.dim() == 2:
        extra = wav_mask.size(-1) % latent_t_size
        if extra > 0:
            wav_mask = wav_mask[:, :-extra]
        wav_mask = wav_mask.view(wav_mask.size(0), latent_t_size, -1)
        latent_mask = wav_mask.all(-1).to(wav_mask.dtype)
    else:
        raise ValueError("wav_mask must be 1D or 2D")

    return latent_mask

def avg_pool_with_mask(x, mask=None, time_dim=2, reduce_dim=(1, 2, 3), eps=1e-9):
    """Average pooling with mask.
    """
    if mask is None:
        return x.mean(reduce_dim)

    assert time_dim in reduce_dim
    assert x.shape[time_dim] == mask.shape[-1]
    assert mask.dim() == 2, mask.shape
    ...
    if x.dim() == 3:
        if time_dim == 1:
            ...
            raise ValueError("time_dim must be 1 or 2 when x.dim() == 3")
    elif x.dim() == 4:
        if time_dim == 1:
            ...
            raise ValueError("time_dim must be 1, 2 or 3 when x.dim() == 4")
    else:
        raise ValueError("x.dim() must be 3 or 4")
    
    x = x * mask
    x = x.sum(reduce_dim) / (mask.sum(reduce_dim) * d + eps)

    return x

def data_filter(data: list):
    # duration filter
    if "duration" in data[0].keys():
        ...
        for d in data:
            if d["duration"] < 2:
                continue
            _data.append(d)
        print(f"Filtering data with duration < 2, initial data size: {len(data)}, after filtering: {len(_data)}")
        data = _data
...
def sketch_extractor(curve, window_length=7, polyorder=1, mode="nearest", times=2):
    is_tensor = isinstance(curve, torch.Tensor)
    if is_tensor:
        device = curve.device
        curve = curve.cpu().numpy()
    
    for i in range(times):
        curve = savgol_filter(curve, window_length, polyorder, mode=mode)
        window_length = window_length // 2 + 1
    
    if is_tensor:
        curve = torch.from_numpy(curve).to(device)
    
    return curve

def modify_curve_length(curve, curve_valid_length, target_mask, pad_is_1=True, verbose=False, enable=True):
    '''
    In the case the provided pitch / energy curve is from another sentence, the length of the curve may not match the length of the current sentence to be synthesized.
    This function interpolates the curve to match the length of the current sentence.
    target_mask: 1 for padding, 0 for real data if pad_is_1 is True, vice versa.
    '''
    if enable:
        if curve is None:
            return None

        valid_len = curve_valid_length
        require_len = (target_mask.size(-1) - torch.sum(target_mask, dim=-1)).long() if pad_is_1 else torch.sum(target_mask, dim=-1).long()
        batch_size, target_len = target_mask.size()
        out = torch.full((batch_size, target_len), torch.min(curve), device=curve.device)
        for i in range(batch_size):
            if valid_len[i] != require_len[i]:
                if verbose:
                    print("interpolating curve from length {} to length {}".format(valid_len[i], require_len[i]))
                if require_len[i] == 0:
                    require_len[i] = 1
                    print("Warning: required curve length is 0, set to 1")
                c = curve[i, :valid_len[i]]
                c = c[None, None, :]
                c = F.interpolate(c, size=require_len[i], mode="linear", align_corners=True).squeeze(0).squeeze(0)
                out[i, :require_len[i]] = c
            else:
                out[i, :require_len[i]] = curve[i, :valid_len[i]]
        return out
    else:
        return curve

def min_max_normalize(x: torch.Tensor, global_min_val=None, global_max_val=None):
