    ...
    for j, style in enumerate(styles):
        ...
        for k, syntext in enumerate(synTexts):
            if j == 0:
                ...
                gd_wav_path = os.path.join(DATA_DIR, text_phoneme_dict[syntext][1])
...
def main(
    ...
    ####### 2. Statistics psd and mc given json file  #######
    ...
    if start_step <= 4 <= end_step and len(cmp_modelnames) > 1:
        ...
        psd_compare_png = os.path.join(out_dir,
    ...
    if start_step <= 5 <= end_step:
        ...
        dual_rmse_utmos_png = os.path.join(out_dir, f"vis_dual_rmse_utmos.pdf")
        vis_dual_utmos_rmse(dul_axis_data, out_pic=dual_rmse_utmos_png)

        guid_masks_png = os.path.join(out_dir, f"vis_guid_masks.pdf")
        ...
        for j, max_len in enumerate(mono_guide_pics["y"]):
            for i, sigma in enumerate(mono_guide_pics["x"]):
                ...
                vis_mono_guide_masks_png = os.path.join(out_dir, f"vis_mono_masks_x{i}_y{j}.png")
...
if __name__ == "__main__":
    ...
    """
    mname_config = {
        "drawspeech_ljspeech_22k_infer": "drawspeech/config/drawspeech_ljspeech_22k_infer.yaml",
        "lddpm_dit_pe_libritts": "drawspeech/config/drawspeech_libritts_dit_16k_cutdur_infer.yaml",  # exp_group_name=config, exp_name=draw...
        "lddpm_mdit_pe_libritts": "drawspeech/config/drawspeech_libritts_dit_24k.yaml",
        "lddpm_dit_pe_ljspeechesd": "drawspeech/config/drawspeech_ljspeechesd_dit_16k_infer.yaml",
        "drawspeech_esd_dit_16k_cutdur_phase2_infer": "drawspeech/config/drawspeech_esd_dit_16k_cutdur_phase2_infer.yaml",
        "drawspeech_libritts_16k_cutdur": "drawspeech/config/drawspeech_libritts_16k_cutdur.yaml",
        "drawspeech_libritts_mdit_16k_cutdur_phase2": "drawspeech/config/drawspeech_libritts_mdit_16k_cutdur_phase2_infer.yaml",
        "drawspeech_libritts_16k_spk_cutdur": "drawspeech/config/drawspeech_libritts_16k_spk_cutdur_infer.yaml",
    }
    """

    #orderd_cmp_modelnames = ["reference", "styletts2", "lddpm_dit_pe_libritts"]
    orderd_cmp_modelnames = ["reference", "lddpm_dit_pe_ljspeechesd", "lddpm_dit_pe_libritts"]
    ...
    DATA_DIR = "/hdd/ESD"   # LibriTTS_16k  ESD
    # OUTPUT
    out_dir = "/hdd/drawspeech/log/exp/lddpm_esd_basic"   # esd

    # INPUT -> syn_styles (emo, spk, wav_p, psd_code), synTexts, and vis related ()
    #TEST_PART_NUM = 2
    ref_json = {
        "esd":"/home/rosen/Project/DrawSpeech_PyTorch/data/dataset/metadata/esd/train.json",
        "libritts": "/home/rosen/Project/DrawSpeech_PyTorch/data/dataset/metadata/libritts16k_cutdur/train.json"
    }
    ...
    vis_attn_config = {
        "esd": {"show_spk": "0019", "show_t": 0, "show_h": 0, "show_txt": 3, "show_emo": "Surprise", "tick_gran": "syllable"},
        "libritts": {"show_spk": "?", "show_t": 0, "show_h": 0, "show_txt": 3, "show_emo": "Neutral", "tick_gran": "syllable"}
    }
    vis_psd_config = {
        "esd": {"show_spk": "0013", "show_emo": "Surprise", "tick_gran": "syllable", "show_txt": (3, 4)},
        "libritts": {"show_spk": "0013", "show_emo": "Surprise", "tick_gran": "syllable", "show_txt": (3, 4)}
    }  # ref3:  15 - 19
    vis_psdmel_config = {
    ...
    # INPUT
    eval_models = ["lddpm_dit_pe_ljspeechesd", "lddpm_dit_pe_libritts"]  # ["cfm_dit_self", "cfm_dit_cross_distgl", "cfm_mdit_cross_distgl", "styletts2"]
    #eval_models = ["lddpm_dit_pe_libritts"]
    eval_models = ["lddpm_dit_pe_ljspeechesd", "lddpm_dit_pe_libritts"]
    #eval_models = ["drawspeech_libritts_16k_spk_cutdur", "drawspeech_libritts_mdit_16k_cutdur_phase2"]
    eval_models = ["styleTTS2"]

    #eval_models = ["drawspeech_ljspeech_22k_infer"]
    #eval_models = ["drawspeech_libritts_mdit_16k_cutdur_phase2"]
